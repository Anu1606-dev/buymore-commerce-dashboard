(function () {
  ...
  const cards = Array.from(
    root.querySelectorAll(".product-card[data-category]"),
  );
  const categoryTabs = Array.from(
    root.querySelectorAll(".category-tabs .category-tab"),
  );
  ...
  const sectionMeta = {
    explore: {
      title: "Explore",
      eyebrow: "Fresh picks for your wardrobe",
      products: [
        {
          ...
          category: "women",
          price: 36,
          image: catalogue["WMX Rubber Zebra sandal"].image,
        },
        {
          ...
          category: "men",
          price: 89,
          image: catalogue["Super Skinny jogger in brown"].image,
        },
      ],
    },
    popular: {
      title: "Popular Products",
      eyebrow: "Loved by the BuyMore community",
      products: [
        {
          name: "WMX Rubber Zebra sandal",
          label: "Trending now",
    ...
    clothing: {
      ...
      eyebrow: "Everyday layers, elevated",
      products: [
        {
          name: "Layered linen look",
          label: "New arrival",
          category: "women",
          price: 74,
          image:
            "https://images.unsplash.com/photo-1529139574466-a303027c1d8b?w=550&q=85",
        },
        {
          ...
          price: 96,
          image:
            "https://images.unsplash.com/photo-1495555961986-6d4c1ecb7be3?w=550&q=85",
        },
      ],
    },
    gifts: {
      title: "Gifts and Living",
      eyebrow: "Small details, big delight",
      products: [
        {
          name: "Ceramic accent lamp",
          label: "Gift pick",
          category: "women",
          price: 42,
          image:
            "https://images.unsplash.com/photo-1540932239986-30128078f3c5?w=550&q=85",
        },
        {
          name: "Soft lounge throw",
          label: "Home edit",
          category: "men",
          price: 58,
          image:
            "https://images.unsplash.com/photo-1586023492125-27b2c045efd7?w=550&q=85",
        },
      ],
    },
    inspiration: {
      title: "Inspiration",
      eyebrow: "Ideas for your next look",
      products: [
        {
          name: "Bold knit styling",
          label: "Style story",
          category: "women",
          price: 68,
          image:
            "https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?w=550&q=85",
        },
        {
          name: "Weekend utility set",
          label: "Lookbook",
          category: "men",
          price: 105,
          image:
            "https://images.unsplash.com/photo-1551488831-00ddcb6c6bd3?w=550&q=85",
        },
      ],
    },
  };
  ...
  function renderCart() {
    const count = state.cart.reduce(function (n, item) {
      return n + item.quantity;
    }, 0);
    const subtotal = state.cart.reduce(function (n, item) {
      return n + item.price * item.quantity;
    }, 0);
    root.querySelectorAll("[data-cart-count]").forEach(function (node) {
    ...
    list.innerHTML = state.cart.length
      ? ""
      : '<div class="cart-empty">Your cart is waiting for a great find.</div>';
    state.cart.forEach(function (item) {
      const row = document.createElement("div");
      row.className = "cart-item";
      row.innerHTML =
        '<div class="cart-item-image"></div><div><strong>' +
        item.name +
        "</strong><small>" +
        item.quantity +
        " × " +
        money(item.price) +
        '</small></div><button class="cart-item-remove" type="button" data-remove="' +
        item.name +
        '">×</button>';
      row.querySelector(".cart-item-image").style.backgroundImage =
        "url('" + item.image + "')";
    ...
    list.querySelectorAll("[data-remove]").forEach(function (button) {
      button.addEventListener("click", function () {
        state.cart = state.cart.filter(function (item) {
          return item.name !== button.dataset.remove;
        });
        saveState();
        renderCart();
  ...
  function setSection(section, announce) {
    ...
    productCards.forEach(function (card, index) {
      const product = view.products[index];
      if (!product) return;
      card.dataset.category = product.category;
      card.dataset.price = product.price;
      card.dataset.name = product.name;
      card.querySelector(".product-image").style.backgroundImage =
        "url('" + product.image + "')";
      ...
      const priceButton = card.querySelector("[data-add]");
      priceButton.textContent = money(product.price);
      priceButton.dataset.product = product.name;
      priceButton.dataset.price = product.price;
      priceButton.dataset.image = product.image;
    });
    category = "all";
  ...
  root.querySelectorAll(".catalogue-grid article").forEach(function (card) {
    ...
    card.addEventListener("click", function (event) {
      if (!event.target.closest("button, a, input")) activateCard();
    });
  ...
  root.querySelectorAll("[data-sort]").forEach(function (button) {
    button.addEventListener("click", function () {
      const row = root.querySelector(".product-row");
      cards
        .slice()
        .sort(function (a, b) {
          return button.dataset.sort === "low"
  ...
  root.querySelectorAll("[data-favourite]").forEach(function (button) {
    button.addEventListener("click", function () {
      ...
      button.textContent = button.classList.contains("is-favourite")
        ...
        : "♡";
      showToast(
        button.classList.contains("is-favourite")
          ? "Added to favourites"
  ...
  root.querySelectorAll("[data-add]").forEach(function (button) {
    button.addEventListener("click", function () {
      ...
      const details = catalogue[name] || {
        price:
          Number(button.dataset.price) ||
          Number(button.textContent.replace(/[^0-9.]/g, "")) ||
          0,
        image: button.dataset.image || "",
      };
      const current = state.cart.find(function (item) {
  ...
  root.querySelector("[data-cart]")?.addEventListener("click", openCart);
  document
    .querySelector("[data-cart-close]")
    ?.addEventListener("click", closeCart);
  document
    .querySelector("[data-cart-backdrop]")
    ?.addEventListener("click", closeCart);
  document
  ...
  root.querySelectorAll("[data-account]").forEach(function (option) {
    option.addEventListener("click", function () {
      root.querySelector("[data-account-name]").textContent =
        option.dataset.account;
      root.querySelector(".profile-avatar").style.backgroundImage =
        "url('" + option.dataset.avatar + "')";
      root.querySelectorAll("[data-account]").forEach(function (item) {
        item.classList.toggle("is-current", item === option);
      });
      menu.hidden = true;
      toggle.setAttribute("aria-expanded", "false");
      toggle.classList.remove("is-open");
      state.account = option.dataset.account;
      saveState();
      showToast("Switched to " + option.dataset.account);
    });
  });
  root.querySelectorAll("[data-view-tab]").forEach(function (tab) {
    tab.addEventListener("click", function () {
      root.querySelectorAll("[data-view-tab]").forEach(function (item) {
        item.classList.toggle("is-active", item === tab);
        item.setAttribute("aria-selected", item === tab ? "true" : "false");
      });
      showToast(
        tab.dataset.viewTab === "website"
          ? "Website preview selected"
          : "Dashboard selected",
      );
    });
  });
  const initialSection = sectionMeta[window.location.hash.slice(1)]
